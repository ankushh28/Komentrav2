...
const FROM = process.env.EMAIL_FROM || 'ReplyPilot <onboarding@resend.dev>';
  ...
  <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 540px; margin: 0 auto; padding: 32px;">
    <div style="text-align: center; margin-bottom: 24px;">
      <div style="display:inline-flex; align-items:center; gap:8px;">
        ...
        <span style="font-size:22px;font-weight:800;letter-spacing:-0.5px;">ReplyPilot</span>
      </div>
    </div>
    ...
    <p style="margin-top:24px;text-align:center;color:#94a3b8;font-size:12px;">© ReplyPilot · Instagram Comment Automation</p>
  </div>`;

  const r = await resend.emails.send({
    from: FROM,
    to: toEmail,
    subject: `${otp} is your ReplyPilot verification code`,
    html,
  });
  return r;
}
